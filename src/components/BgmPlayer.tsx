...
function getInitialVolume() {
  ...
  const savedVol = Number(window.localStorage.getItem("bgm_vol") || "0.25");
  return Number.isFinite(savedVol) ? Math.min(1, Math.max(0, savedVol)) : 0.25;
}
...
export function BgmPlayer({ src = DEFAULT_SRC, title = "BGM" }: Props) {
  ...
  const [vol, setVol] = useState(getInitialVolume);
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[60]">
      ...
      <div className="flex flex-col items-end gap-2">
        {open ? (
          <div className="surface-card w-[260px] p-4">
            ...
            <p className="mt-2 text-xs leading-5 text-zinc-600">
              브라우저 정책상 자동 재생은 막혀 있어요. 버튼으로 재생을 시작해 주세요. (파일이 없으면 재생되지 않습니다.)
            </p>
            <label className="mt-3 block text-xs font-medium text-zinc-700">볼륨: {(vol * 100).toFixed(0)}%</label>
            <input
              type="range"
              min={0}
              max={1}
            ...
            <div className="mt-3 flex gap-2">
              <button type="button" onClick={toggle} className="button-primary flex-1">
                {playing ? "일시정지" : "재생"}
              </button>
              <button
                type="button"
